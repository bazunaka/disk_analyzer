...
public class App extends Application {
    ...
    public void start(Stage stage) throws Exception {
        ...
        Button button = new Button("Выбрать директорию"); //визуализация ожидания
        button.setOnAction(event -> {
            File file = new DirectoryChooser().showDialog(stage); //file = директория тут
            String path = file.getAbsolutePath();
            sizes = new Analyzer().calculateDirectorySize(Path.of(path));
            buildChart(path); //отображение результата
        });
